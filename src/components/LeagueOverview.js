...
const LeagueOverview = ({
  ...
  useEffect(() => {
    const fetchLeagueData = async () => {
      try {
        ...
        const [leagueData, usersData, rostersData, bracketData] =
          await Promise.all([
  ...
  const processPlayoffBracketData = (
    bracketData,
    processedTeams,
    rostersData,
    leagueType
          ...
          teams.map((team, index) => {
            ...
            return (
              <TeamCard
                ...
                rank={position}
                isPromotionZone={
                  promotionZone.includes(position) || isSpecialPromotion
                }
                isRelegationZone={
                  relegationZone.includes(position) || isSpecialRelegation
                }
        ...
        <div className="flex flex-wrap gap-4 text-sm">
          {winnersBracketChampion && (
            <div className="flex items-center">
              <span className="w-5 h-5 flex items-center justify-center text-yellow-500 mr-1">
                🏆
              </span>
          ...
          {specialPromotionTeam &&
            specialPromotionTeam !== winnersBracketChampion &&
